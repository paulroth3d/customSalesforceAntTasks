...
/**
 *  Task that removes second package from the first package
**/
public class SFDC_SubtractCollapsed extends Task {
			...
			while( (line = reader.readLine()) != null){
				...
				if( !("").equals( line )){
					...
					if( lineMap.containsKey( lowerLine )){
						System.out.println( "removing [" + lowerLine + "]" );
						lineMap.remove( lowerLine );
					} else {
						if( isChatty ) System.out.println( "line NOT FOUND [" + lowerLine + "]" );
					}
				}
			}
